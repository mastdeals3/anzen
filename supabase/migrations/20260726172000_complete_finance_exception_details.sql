...
WITH exceptions AS (
  SELECT e.*,
    CASE
      WHEN e.reason ILIKE '%USD%rate%'
        ...
        OR e.reason ILIKE '%without an accrued payable%'
      ...
      WHEN e.reason ILIKE '%No unique authoritative relationship%'
        ...
        OR e.reason ILIKE '%missing Faktur Pajak%'
  ...
  SELECT
    ...
    COALESCE(
      ...
      sales_customer.company_name,
      purchase_supplier.company_name,
      l.counterparty_name,
  ...
  LEFT JOIN public.sales_invoices si
    ON e.document_type = 'sales_invoice' AND si.id = e.document_id
  LEFT JOIN public.purchase_invoices pi
    ON e.document_type = 'purchase_invoice' AND pi.id = e.document_id
  ...
  LEFT JOIN public.customers sales_customer ON sales_customer.id = si.customer_id
  LEFT JOIN public.suppliers purchase_supplier ON purchase_supplier.id = pi.supplier_id
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE
      ...
      OR (e.document_type = 'sales_invoice' AND j.id = si.journal_entry_id)
      OR (e.document_type = 'purchase_invoice' AND j.id = pi.journal_entry_id)
