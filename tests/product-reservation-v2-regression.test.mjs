...
test('ATP remains invariant while ambiguous legacy reservations are held',()=>{assert.match(sql,/FROM public\.stock_reservations sr/);assert.match(sql,/sr\.status='active' AND NOT sr\.is_released/);assert.match(sql,/sr\.id=ANY\(migrated\.legacy_reservation_ids\)/);});
...
test('migration preview is quantity-neutral',()=>{const safe=plan.reservations.filter(r=>r.migration_action==='CONSOLIDATE_TO_PRODUCT_RESERVATION');const holds=plan.reservations.filter(r=>r.migration_action==='HOLD_AMBIGUOUS_PENDING_DC');const safeByItem=new Map();for(const row of safe)safeByItem.set(row.so_item_id,(safeByItem.get(row.so_item_id)||0)+row.current_quantity);for(const row of safe)assert.equal(safeByItem.get(row.so_item_id),row.proposed_product_level_reservation);assert.equal(safe.reduce((n,r)=>n+r.current_quantity,0)+holds.reduce((n,r)=>n+r.current_quantity,0),16630);assert.deepEqual(plan.proof_totals,{physical_stock_delta:0,delivered_quantity_delta:0,active_commitment_before:16630,active_commitment_after:16630,atp_delta:0,outstanding_so_quantity_delta:0,pending_dc_quantity_delta:0});});
